...
train_dataset = []
validation_dataset = []
for i in range(index_train+1):
    dataset.append(datasetloader(save_root_path=params["save_root_path"],
                  option="train",
                  index=index_train
                  ))
    split_dataset = random_split(dataset, [int(np.floor(len(dataset) * params["validation_ratio"])),
                                           int(np.ceil(len(dataset) * (1 - params["validation_ratio"])))]
                                 )
    train_dataset.append(split_dataset[0])
    validation_dataset.append(split_dataset[1])
...
test_dataset = []
for i in range(index_test+1):
    test_dataset.append(datasetloader(save_root_path=params["save_root_path"],
                                 option="test",
                                 index=index_test
                                 ))
if __TIME__:
...
if __TIME__:
    start_time = time.time()
for i in range(index_train+1):
    train_loader = DataLoader(train_dataset[i], batch_size=params["train_batch_size"],
                              shuffle=params["train_shuffle"])
    validation_loader = DataLoader(validation_dataset[i], batch_size=params["train_batch_size"],
                                   shuffle=params["train_shuffle"])
for i in range(index_test+1):
    test_loader = DataLoader(test_dataset[i], batch_size=params["test_batch_size"],
...
for epoch in range(hyper_params["epochs"]):
    ...
    '''train loader iteration expectation'''
    for i in range(index_train+1):
        with tqdm(train_loader[i], desc="Train ", total=len(train_loader)) as tepoch:
            model.train()
            running_loss = 0.0
            i = 0
    ...
    '''valid iter expect'''
    for i in range(index_train+1):
        with tqdm(validation_loader[i], desc="Validation ", total=len(validation_loader)) as tepoch:
            model.eval()
            running_loss = 0.0
            with torch.no_grad():
                for inputs, target in tepoch:
        ...
        '''test loader iteration expectation'''
        for i in range(index_test+1):
            with tqdm(test_loader[i], desc="test ", total=len(test_loader)) as tepoch:
                model.eval()
                inference_array = []
                target_array = []
                with torch.no_grad():
